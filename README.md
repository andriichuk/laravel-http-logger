...
```bash
php artisan vendor:publish --tag="laravel-http-logger-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="laravel-http-logger-views"
```

## Usage

```php
$HttpLogger = new Andriichuk\HttpLogger();
echo $HttpLogger->echoPhrase('Hello, Andriichuk!');
```
